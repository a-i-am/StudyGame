...
namespace StudyGame.Editor
{
    public class StudyGameDirectorHub : EditorWindow
    {
        ...
        [MenuItem("StudyGame/Director Hub (Legacy)")]
        ...
        {
            GUILayout.Label("📄 Anomaly Data Importer & Scenario Importer", EditorStyles.boldLabel);
            EditorGUILayout.HelpBox("수능 기믹 JSON 파일과 마크다운 시나리오를 파싱하여 에셋으로 굽습니다.", MessageType.Info);
            ...
            GUILayout.Space(10);
            if (GUILayout.Button("Bake All Markdown Scenarios (Auto)", GUILayout.Height(40)))
            {
                BakeAllScenarios();
            }
        }
        ...
        private void DrawTestRunnerTab()
        {
            ...
            EditorGUILayout.HelpBox("테스트 씬(`.unity`) 파일들을 열어 직접 플레이합니다.", MessageType.Info);

            GUILayout.Space(10);
            EditorGUILayout.BeginHorizontal();

            if (GUILayout.Button("🕹️ Open PlayTest Scene", GUILayout.Height(40)))
            {
                if (EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo())
                {
                    EditorSceneManager.OpenScene("Assets/Scenes/VerificationTestScene.unity");
                }
            }

            if (GUILayout.Button("📜 Open Sandbox Scene", GUILayout.Height(40)))
            {
                if (EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo())
                {
                    EditorSceneManager.OpenScene("Assets/Scenes/Scene_ExplorationRunner.unity");
                }
            }
        ...
        private void BakeAllScenarios()
        {
            ...
            foreach (var file in files)
            {
                ...
                string epName = filename.Split('_')[0].ToUpper();
                GenerateEpisodeNode(text, outDir, epName, true);
            }
            Debug.Log($"Baked {files.Length} scenarios successfully.");
        }

        private static void GenerateEpisodeNode(string markdown, string outDir, string epName, bool silent = false)
        {
