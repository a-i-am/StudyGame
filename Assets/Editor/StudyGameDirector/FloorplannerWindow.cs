...
{
    public class FloorplannerWindow : EditorWindow
    {
        ...
        private Vector2 _masterCameraRotation = new Vector2(45, 0);
        ...
        public static void Open(string floorplanJsonPath = null)
        {
            var wnd = GetWindow<FloorplannerWindow>();
            wnd.titleContent = new GUIContent("🏗️ Floor Planner");
            wnd.minSize = new Vector2(800, 600);
            if (!string.IsNullOrEmpty(floorplanJsonPath) && System.IO.File.Exists(floorplanJsonPath))
            {
                wnd.LoadFromPath(floorplanJsonPath);
            }
            wnd.Show();
        }

        public LevelGeometryData GetGeometry() => _levelGeometry;
        ...
        private void LoadFromPath(string path)
        {
            ...
            foreach (var n in data.Nodes)
            {
                ...
                node.SetWorldPosition(n.WorldPosition);
                _graphView.AddElement(node);
            }
        ...
        private void DrawFloorplanMonitor()
        {
            ...
            float s = Mathf.Max(0.1f, _levelGeometry.GridSize);
            Vector3 center = _floorplanCameraPivot;
            float camSize = _floorplanCameraSize;

            for (float x = Mathf.Floor(center.x - camSize); x <= Mathf.Ceil(center.x + camSize); x += s)
            {
            ...
            for (float z = Mathf.Floor(center.z - camSize); z <= Mathf.Ceil(center.z + camSize); z += s)
            {
